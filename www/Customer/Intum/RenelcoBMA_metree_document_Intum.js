...
var Metree_Document = (function () {
	var Metree_Document = function (metree_id, document_id, metree_document_type_id) {
...
//				alert('NOT a KitKat Device...');
				...
				this.Metree_DocumentEvent.initCustomEvent('Metree_Document', true, false, {metree_id: 0, document_id: 0, metree_document_type_id: 0, time: new Date()});
			}
		}
		...
		var thisMetree_Document = this;
			...
			if(LOCAL_DB==false) {
				...
				databaseManager.localDB.transaction(function(tx) {
					tx.executeSql('INSERT INTO METREE_DOCUMENT (METREE_ID, METREE_DOCUMENT_TYPE_ID, DOCUMENT_ID) VALUES (?, ?, ?)', 
...
//		alert('Metree_Document.update()');	
		var thisMetree_Document = this;
			...
			if(LOCAL_DB==false) {
				...
				databaseManager.localDB.transaction(function(tx) {
					tx.executeSql('DELETE FROM METREE_DOCUMENT WHERE (METREE_ID = ? AND METREE_DOCUMENT_TYPE_ID = ? AND DOCUMENT_ID = ?)', 
						      	  [thisMetree_Document.metree_id,
						           thisMetree_Document.metree_document_type_id,
						           thisMetree_Document.document_id], 
						          function() {
...
//		alert('load');
	...
	//				        alert(JSON.stringify(item));
				...
				databaseManager.localDB.transaction(function(tx) {
					tx.executeSql('SELECT METREE_ID, METREE_DOCUMENT_TYPE_ID, DOCUMENT_ID  FROM METREE_DOCUMENT WHERE METREE_ID = ?',
						      [id], 
						      function (tx, rs) {
							  	var data = rs.rows;
							  	var key = 0;
