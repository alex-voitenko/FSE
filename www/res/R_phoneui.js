...
$(document).ready(function() {
	...
	function preProcess(context) {
		...
		hiddenSelectToSLM = function(v, context) {
			var sel = $("#" + $(v).attr("data-hiddenInputId"), context);
//			var verboseItemsInfo = $(v).attr("data-verbose-items-info") == "true";  // HACKED by Renelco: generally set to "true"
			var verboseItemsInfo = false;  											// HACKED by Renelco: generally set to "true"
...
//							selInfoId.text("" + labelsArray.length);				// HACKED by Renelco: we display our own info
							selInfoId.text("");
						}
					}
				}
	...
	if (!phoneui.showActivityDialog.controller) {
		...
		var draw = function(ctx, offset) {
			...
			for(var i = 0; i<bars; i++){
				...
				var op = (1 + i)/bars;
				ctx.fillStyle = "rgba(255, 128, 64, " + op*op*op + ")";	/* Modified by Renelco, previous rgba(255, 255, 255, " + op*op*op + ") */
		...
		phoneui.showActivityDialog.controller = {
			...
			start: function (){
				this.timer = setInterval(nextAnimation, 100); // Hacked by Renelco: previously 20 fps
			}
		};
	}
